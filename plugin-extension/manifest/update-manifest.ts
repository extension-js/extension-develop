...
function transformManifest (api: RsbuildPluginAPI) {
  ...
  const defaultOutputManifest = JSON.parse(fs.readFileSync(manifestOutputpath).toString())
  const compiledEntries: Record<string, any> = defaultOutputManifest.entries
  const publicFolder = typeof api.getRsbuildConfig().server?.publicDir === 'boolean'
    ? 'public/'
    : (api.getRsbuildConfig().server?.publicDir as Record<string, string>).name + '/'
