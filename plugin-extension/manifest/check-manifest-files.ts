...
const handleHtmlErrors = (api: RsbuildPluginAPI) => {
  const htmlFields: Record<string, string> =
    api.useExposed('manifest-fields')().html;
...
const handleIconsErrors = (api: RsbuildPluginAPI) => {
  const iconsFields: Record<string, any> =
    api.useExposed('manifest-fields')().icons;
...
const handleJsonErrors = (api: RsbuildPluginAPI) => {
  const jsonFields: Record<string, any> =
    api.useExposed('manifest-fields')().json;
...
const handleScriptsErrors = (api: RsbuildPluginAPI) => {
  const scriptsFields: Record<string, any> =
    api.useExposed('manifest-fields')().scripts;
